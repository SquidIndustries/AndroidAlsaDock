...
#This script registers a HID media keyboard with a Android device that is in 
#audio output mode (0x2D02 - audio) as define by Android Open Accessory Protocol 2.0
#Then sends a HID command to Android device. The un-registers HID device. 
...
#hard code vendor and product id since its always the same
dev = usb.core.find(idVendor=int("0x18d1", 16), idProduct=int("0x2d02", 16))
...
#uncomment the  command you want to send
